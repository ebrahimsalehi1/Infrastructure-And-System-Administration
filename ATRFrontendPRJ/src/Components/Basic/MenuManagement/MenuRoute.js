...
import {MENU_CREATE, MENU_DELETE, MENU_UPDATE} from "../../../Utils/Config/RouterServices/Menu";
import {APPLICATION_GET} from "../../../Utils/Config/RouterServices/Application";
import {withRouter} from 'react-router-dom';
import {CreateApi, DeleteApi, ReadApi, UpdateApi} from "../../../Utils/Config/CallServiceRouter";
...
function Menu(props) {
    ...
    const   {whichCompShow,rowData,onClose,onSaveSuccess,onSaveFailed,onLoadFailed,onCancelClick,isAdmin,columns,title} = props?.history?.location?.state;
    ...
    console.log('MenuRoute is rendering',props.location.state.data);
                ...
                <Grid container spacing={0}>
                    ...
                    <Grid item md={12} xs={12} style={{textAlign:'left'}}>
                        {(whichCompShow==='add' || whichCompShow==='add_root') ?
                            ...
                            :
                            <Button variant={"contained"} color={"secondary"} className={classes.root} onClick={async ()=>{
                        ...
                        <Button variant={"contained"} color={"secondary"} className={classes.root}  onClick={onCancelClick} > بستن </Button>
                                        ...
                                        await DeleteApi( {url:`${MENU_DELETE.URL}${menuId}`})
                                            .then(result => {
                                                ...
                                                switch(result.status){
                                                    case 200:
                                                        showSnack(result.data && result.data.length>0 ? result.data.join(",") : "حذف اطلاعات با موفقیت انجام شد", "success", true, 3000);
                                                        onSaveSuccess();
                                                        break;
                                                    case 204:
                                                        showSnack(
